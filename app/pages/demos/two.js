...
// import { auth_group_list } from 'actions/auth';
import { TPostData, urlBase } from 'utils/TAjax';
import SimpleTable from 'components/TTable/SimpleTable';
import { CreateModal, UpdateModal } from 'components/TModal';
import { fn_mes_trans } from 'functions'
// import { SimpleQForm, StandardQForm } from 'components/TForm';
import TUserDetails from '../TUser/TUserDetails';
...
export default class TUserList extends Component {
    ...
    handleCreat = ( data ) => {
        const { uGroupUUID } = data;
        delete data.uGroupUUID;
        const addData = {
            cols: fn_mes_trans.toCols( data ),
        }
        console.log( '开始添加', addData, data );
        this
            .props
            .dispatch( account_add( addData, ( respose ) => {
                const cond = {
                    cols: fn_mes_trans.toCols( { uUserUUID: respose.data.uuid, uGroupUUID: uGroupUUID } ),
                }
                console.log( '添加账户成功！', respose, cond );
                this
                    .props
                    .dispatch( auth_config_add( cond, respose => console.log( '添加配置成功！', respose ) ) )
            } ) )
    }
    ...
    handleUpdate = ( data ) => {
        const { uGroupUUID } = data;
        delete data.uGroupUUID;
        const { uObjectUUID } = this.state.updateFromItem;
        const editData = {
            uuid: uObjectUUID,
            cols: fn_mes_trans.toCols( data ),
        }
        ...
        // const config_filter = {
        //     fn_mes_trans.toFilter( { uUserUUID: uObjectUUID } ),
        // }
        this
            .props
            .dispatch( auth_config_list( fn_mes_trans.toFilter( { uUserUUID: uObjectUUID } ), ( respose ) => {
                console.log( '查找到config项', respose )
                const UUID = respose.data.list[0].uObjectUUID
                const cond = {
                    uuid: UUID,
                    cols: fn_mes_trans.toCols( { uUserUUID: uObjectUUID, uGroupUUID: uGroupUUID } ),
                }
                this
                    .props
                    .dispatch( auth_config_update( cond, respose => console.log( '修改配置成功！', respose ) ) )
            } ) )
    }
        ...
        const UFormItem = [
            {
                ...
                placeholder: '请输入用户编号',
                rules: [{ required: true, message: '编号至少为 1 个字符' }],
                options: this.props.UserAccount.authGPList,
            }, {
                name: 'strAvatar',
                label: '图片',
                type: 'antUpload',

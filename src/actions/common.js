import {
    createAction,
} from 'redux-actions'
import {
    common,
} from 'api'
import {
    createAjaxAction, ajax,
} from 'utils'


// export const requestAmList = createAction('request am list')
// export const recevieAmList = createAction('receive am list')
// export const fetchAmList = createAjaxAction(common.amList, requestAmList, recevieAmList)
// export const resetAmList = createAction('reset am list')
export const fetchLogin = createAjaxAction( common.login )
export const fetchRegister = createAjaxAction( common.register )
export const userInfo = createAjaxAction( common.userInfo )
export const updateBreadcrumbList = createAction( 'update breadcrumb list' );

// export const device_brand_list = ajax.fetchJSONByPost( 'api/device/brand/list' )
export const fetchEGTimeGraph = createAjaxAction(
    ajax.fetchJSONByPost( '/api/datareceive/realtimeGraph' ),
    null,
    createAction( 'succReceiveEGTimeGr' ),
)

export const fetchEGBarGraph = createAjaxAction(
    ajax.fetchJSONByPost( '/api/datareceive/reports' ),
    createAction( 'requestEGBarGr' ),
    createAction( 'succReceiveEGBarGr' ),
)
export const fetchABLineGraph = createAjaxAction(
    ajax.fetchJSONByPost( '/api/datareceive/environment' ),
    createAction( 'requestABLineGr' ),
    createAction( 'succReceiveABLineGr' ),
)
export const fetchABLineGraph_all = createAjaxAction(
    ajax.fetchJSONByPost( '/api/datareceive/environment' ),
    createAction( 'requestABLineGr' ),
    createAction( 'succReceiveABLineGr_all' ),
)

export const resMqtt = createAction( 'resMqttData' );
